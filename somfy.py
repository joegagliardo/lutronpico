...
class SomfyMyLinkTarget:
    ...
    async def up1(self, delay = 1):
        import time
        await self._somfy._up(self._target_id)
        time.sleep(delay)
        return await self._somfy._stop(self._target_id)

    async def down1(self, delay = 1):
        import time
        await self._somfy._down(self._target_id)
        time.sleep(delay)
        return await self._somfy._stop(self._target_id)

async def controlShade(shade = 'study', command = 'down'):
    shades = {
                'north' : 'CC10B126.1'
                ,'east' : 'CC10B126.2'
                ,'bedroom' : 'CC10B126.3'
    ...
    somfy = SomfyMyLink('Shades', '10.0.0.95')
    cover = somfy.target(shades.get(shade, shade))
...
def main():
    import sys
    shade = sys.argv[1] if len(sys.argv) >= 2 else 'study'
    command = sys.argv[2] if len(sys.argv) >= 3 else 'down'

    loop = asyncio.get_event_loop()
    loop.run_until_complete(controlShade(shade, command))
    loop.close()

if __name__ == '__main__':
    # import sys
    # sys.argv = ['x','study', 'raise']
    main()
